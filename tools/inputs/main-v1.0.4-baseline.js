...
var init_TranscriptSchema = __esm({
  "src/utils/document/TranscriptSchema.ts"() {
  }
});

// src/utils/document/StructuredOutputRenderer.ts
var StructuredOutputRenderer_exports = {};
__export(StructuredOutputRenderer_exports, {
  DEFAULT_SECTIONS: () => DEFAULT_SECTIONS,
  StructuredOutputRenderer: () => StructuredOutputRenderer
});
...
function renderSection(section) {
  ...
  const startMarker = `<!-- neurovox:ai:start:section=${section.id};hash=${hash} -->`;
  const endMarker = `<!-- neurovox:ai:end:section=${section.id} -->`;
  const contentLines = section.content.split("\n").map((line) => line.trim() ? `> ${line}` : ">").join("\n");
  return [
    `> [!${section.calloutType}]- ${section.heading}`,
    `> ${startMarker}`,
    contentLines,
    `> ${endMarker}`
  ].join("\n");
}
function renderTranscriptCallout(transcript, includeTimestamps, audioFilePath) {
  const text = flattenTranscriptText(transcript, includeTimestamps);
  const contentLines = text.split("\n").map((line) => line.trim() ? `> ${line}` : ">").join("\n");
  const audioLine = audioFilePath ? `> ![[${audioFilePath}]]
>
` : "";
  return [
    "> [!info]- Full Transcript",
    audioLine + contentLines
  ].join("\n");
}
function escapeRegex(str) {
  return str.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
var DEFAULT_SECTIONS, StructuredOutputRenderer;
var init_StructuredOutputRenderer = __esm({
  "src/utils/document/StructuredOutputRenderer.ts"() {
    init_TranscriptSchema();
    DEFAULT_SECTIONS = [
      { id: "summary", heading: "Summary", calloutType: "abstract" },
      { id: "keypoints", heading: "Key Points", calloutType: "note" },
      { id: "actions", heading: "Action Items", calloutType: "todo" },
      { id: "decisions", heading: "Decisions", calloutType: "important" }
    ];
    StructuredOutputRenderer = class {
      /**
       * Renders the complete structured output as markdown.
       */
      render(input) {
        const parts = [];
        parts.push(this.renderHeader(input.meta));
...
${input.userNotes.trim()}`);
        ...
        parts.push(renderTranscriptCallout(
          input.transcript,
          input.includeTimestamps ?? false,
          input.meta.audioFilePath
        ));
        return parts.join("\n\n") + "\n";
      }
      /**
       * Renders the H2 title and metadata line.
       */
      renderHeader(meta) {
        const title = meta.title || "Meeting Notes";
        const metaParts = [];
        if (meta.date) {
          metaParts.push(`**Date:** ${meta.date}`);
        }
        if (meta.duration) {
          metaParts.push(`**Duration:** ${meta.duration}`);
        }
        if (meta.participants && meta.participants.length > 0) {
          metaParts.push(`**Participants:** ${meta.participants.join(", ")}`);
        }
        const metaLine = metaParts.length > 0 ? "\n" + metaParts.join(" | ") : "";
        return `## ${title}${metaLine}`;
      }
      /**
       * Parses AI post-processing output into structured sections.
       *
       * Expects the AI to output markdown with H2 headings matching section names.
       * Falls back gracefully: if a section heading is not found, that section is omitted.
       *
       * This is the bridge between the current single-string postProcessing output
       * and the structured section model. M3 will replace this with template-aware parsing.
       */
      static parseSectionsFromPostProcessing(postProcessingText, sectionDefs = DEFAULT_SECTIONS) {
        const sections = [];
        const lines = postProcessingText.split("\n");
        for (const def of sectionDefs) {
          const headerPattern = new RegExp(
            `^#{1,3}\\s+${escapeRegex(def.heading)}\\s*$`,
            "im"
          );
          let startIdx = -1;
...
// src/utils/templates/MeetingTemplates.ts
var MeetingTemplates_exports = {};
__export(MeetingTemplates_exports, {
  ...
  getMaxTranscriptWords: () => getMaxTranscriptWords
});
function getBuiltInTemplate(id) {
  return BUILT_IN_TEMPLATES.find((t) => t.id === id) ?? BUILT_IN_TEMPLATES[0];
}
function getAllTemplates(userCustomTemplates) {
  const customs = (userCustomTemplates ?? []).filter((t) => !t.isBuiltIn);
  return [...BUILT_IN_TEMPLATES, ...customs];
}
function getMaxTranscriptWords(model) {
  const tokens = MODEL_CONTEXT_LIMITS[model] ?? 16e3;
  const usableTokens = Math.floor(tokens * 0.8);
  const transcriptTokens = Math.max(1e3, usableTokens - 3e3);
  return Math.floor(transcriptTokens * 0.75);
}
var SHARED_SYSTEM_PROMPT, BUILT_IN_TEMPLATES, MODEL_CONTEXT_LIMITS;
var init_MeetingTemplates = __esm({
  "src/utils/templates/MeetingTemplates.ts"() {
    SHARED_SYSTEM_PROMPT = "You are a meeting notes assistant. Extract structured information from the transcript below. Use markdown formatting. Be concise \u2014 bullet points, not paragraphs. Preserve speaker attributions where relevant. Output each section with a markdown heading (## Section Name) exactly as specified.";
    BUILT_IN_TEMPLATES = [
      {
        id: "general",
        ...
        userPromptTemplate: `From the following meeting transcript, extract:

## Summary
2-3 sentence overview of the meeting.

## Key Points
- Bullet points of major topics discussed, with speaker attribution

## Action Items
- [ ] [Owner] Action description (deadline if mentioned)

## Decisions
- Decision description \u2014 rationale if stated

## Follow-ups
- Items that need follow-up but aren't clear action items
...
{transcript}`,
        sections: [
          ...
          { id: "actions", heading: "Action Items", calloutType: "todo" },
          { id: "decisions", heading: "Decisions", calloutType: "important" },
          { id: "followups", heading: "Follow-ups", calloutType: "info" },
          { id: "questions", heading: "Open Questions", calloutType: "tip" }
        ],
        isBuiltIn: true
      },
      {
        id: "one-on-one",
        name: "1:1",
        description: "Manager-report or peer 1:1s",
        systemPrompt: SHARED_SYSTEM_PROMPT,
        userPromptTemplate: `From the following 1:1 meeting transcript, extract:

## Summary
2-3 sentence overview of the 1:1.

## Discussion Points
- Key topics discussed, with context

## Feedback
- Feedback given or received, with speaker attribution

## Action Items
- [ ] [Owner] Action description (deadline if mentioned)

## Next Meeting Topics
- Topics to carry over to the next 1:1

Transcript:
{transcript}`,
        sections: [
          { id: "summary", heading: "Summary", calloutType: "abstract" },
          { id: "discussion", heading: "Discussion Points", calloutType: "note" },
          { id: "feedback", heading: "Feedback", calloutType: "info" },
          { id: "actions", heading: "Action Items", calloutType: "todo" },
          { id: "nexttopics", heading: "Next Meeting Topics", calloutType: "tip" }
        ],
        isBuiltIn: true
      },
      {
        id: "standup",
        name: "Standup",
        description: "Daily standups, sprint ceremonies",
        systemPrompt: SHARED_SYSTEM_PROMPT,
        userPromptTemplate: `From the following standup meeting transcript, extract per-person updates:

## Summary
1-2 sentence overview of the standup.

## What Was Done
- Per-person: what they completed since last standup

## What's Planned
- Per-person: what they plan to work on next

## Blockers
- Any blockers mentioned, with owner and status

## Action Items
- [ ] [Owner] Action to resolve blockers or follow up

Transcript:
{transcript}`,
        sections: [
          { id: "summary", heading: "Summary", calloutType: "abstract" },
          { id: "done", heading: "What Was Done", calloutType: "note" },
          { id: "planned", heading: "What's Planned", calloutType: "info" },
          { id: "blockers", heading: "Blockers", calloutType: "warning" },
          { id: "actions", heading: "Action Items", calloutType: "todo" }
        ],
        isBuiltIn: true
      },
      {
        id: "brainstorm",
        name: "Brainstorm",
        description: "Creative sessions, design discussions",
        systemPrompt: SHARED_SYSTEM_PROMPT,
        userPromptTemplate: `From the following brainstorming session transcript, extract:

## Summary
2-3 sentence overview of the brainstorm session and its goal.

## Problem Statement
- The problem or question being explored

## Ideas Generated
- All ideas proposed, with who proposed them

## Evaluation
- Pros and cons discussed for the key ideas

## Selected Direction
- The direction chosen (if any), with rationale
...
{transcript}`,
        sections: [
          { id: "summary", heading: "Summary", calloutType: "abstract" },
          { id: "problem", heading: "Problem Statement", calloutType: "important" },
          { id: "ideas", heading: "Ideas Generated", calloutType: "note" },
          { id: "evaluation", heading: "Evaluation", calloutType: "info" },
          { id: "direction", heading: "Selected Direction", calloutType: "tip" },
          { id: "nextsteps", heading: "Next Steps", calloutType: "todo" }
        ],
        isBuiltIn: true
      },
      {
        ...
        description: "External-facing meetings",
        systemPrompt: SHARED_SYSTEM_PROMPT,
        userPromptTemplate: `From the following client/external meeting transcript, extract:

## Summary
2-3 sentence overview of the meeting.

## Attendees
- List of participants mentioned or identified

## Discussion Summary
- Key topics discussed, organized by theme

## Commitments Made
- Specific commitments or promises made by either party

## Follow-up Required
- [ ] [Owner] Follow-up action with deadline if mentioned
...
{transcript}`,
        sections: [
          { id: "summary", heading: "Summary", calloutType: "abstract" },
          { id: "attendees", heading: "Attendees", calloutType: "info" },
          { id: "discussion", heading: "Discussion Summary", calloutType: "note" },
          { id: "commitments", heading: "Commitments Made", calloutType: "important" },
          { id: "followup", heading: "Follow-up Required", calloutType: "todo" },
          { id: "questions", heading: "Open Questions", calloutType: "tip" }
        ],
        isBuiltIn: true
      },
      {
        ...
        systemPrompt: SHARED_SYSTEM_PROMPT,
        userPromptTemplate: `From the following interview transcript, extract:

## Summary
2-3 sentence overview of the interview.

## Candidate Info
- Name, role applied for, and any background context mentioned

## Questions Asked
- List of questions asked by the interviewer(s)

## Response Highlights
- Key points from the candidate's responses, organized by question/topic

## Assessment Notes
- Strengths and areas of concern observed

## Decision / Next Steps
- [ ] Hiring decision or next steps discussed

Transcript:
{transcript}`,
        sections: [
          { id: "summary", heading: "Summary", calloutType: "abstract" },
          { id: "candidate", heading: "Candidate Info", calloutType: "info" },
          { id: "questions", heading: "Questions Asked", calloutType: "note" },
          { id: "responses", heading: "Response Highlights", calloutType: "note" },
          { id: "assessment", heading: "Assessment Notes", calloutType: "important" },
          { id: "nextsteps", heading: "Decision / Next Steps", calloutType: "todo" }
        ],
        isBuiltIn: true
      }
    ];
...
var AIProvider = /* @__PURE__ */ ((AIProvider2) => {
  ...
  AIProvider2["Moonshine"] = "moonshine";
  return AIProvider2;
})(AIProvider || {});
var AIModels = {
  ["openai" /* OpenAI */]: [
...
var _AIAdapter = class {
  ...
  async transcribeAudio(audioArrayBuffer, model, _options) {
    try {
      ...
      const audioSizeMB = audioArrayBuffer.byteLength / (1024 * 1024);
      const timeoutMs = Math.max(6e4, audioSizeMB * 1e4);
      try {
        const response = await this.makeAPIRequest(
          endpoint,
          "POST",
        ...
        if (httpError?.response?.status === 400) {
          throw new Error(
            `Invalid request format: ${httpError?.response?.data?.error?.message || "Check audio format and model name"}`
          );
  ...
  async validateApiKey() {
    try {
      ...
      if (!currentKey) {
        this.keyValidated = false;
        this.lastValidatedKey = "";
        return { valid: false, error: "API key is empty", errorType: "auth" };
      }
      if (this.keyValidated && this.lastValidatedKey === currentKey) {
        return { valid: true };
      }
      const result = await this.validateApiKeyImpl();
      if (result.valid) {
      ...
      return { valid: false, error: message, errorType: "unknown" };
    }
  }
...
AIAdapter.DEFAULT_REQUEST_TIMEOUT_MS = 6e4;
...
var DEFAULT_SETTINGS = {
  ...
  postProcessingPrompt: "Extract structured meeting notes from the following transcript. Include sections: Summary, Key Points, Action Items, and Decisions. Use markdown headings (## Section Name) for each section. Be concise \u2014 use bullet points.",
  postProcessingMaxTokens: 2e3,
  ...
  // Meeting Templates (M3)
  defaultMeetingTemplate: "general",
  enableAutoTemplateDetection: false,
  enableEnhanceNotes: true,
  addMeetingFrontmatter: false,
  customMeetingTemplates: "[]",
  // Inbox Auto-Watch
  enableInboxAutoWatch: true,
  autoTranscribeInbox: false,
  archiveAfterTranscribe: false
};
...
var _TranscriptionService = class {
  ...
  async transcribeAudio(audioBuffer, options) {
    ...
    if (!options?.allowEmptyTranscription && (!transcription || !transcription.trim())) {
...
${transcription}`;
  ...
  async validateAdapterWithTimeout(adapter) {
    ...
    const result = await Promise.race([
      adapter.validateApiKey().catch(() => ({ valid: false })),
      new Promise((resolve) => {
        window.setTimeout(() => resolve({ valid: false }), timeoutMs);
      })
    ]);
    return result.valid;
  }
...
var TranscriptRenderer = class {
  ...
  render(input) {
    const transcriptionText = flattenTranscriptText(
      input.transcript,
...
function applySpeakerMappingToEntry(noteContent, entryId, entryStart, entryEnd) {
  ...
  const inferredAliases = inferAliasesFromDiarizedLines(
    entryContent,
...
function findEntryMappingRegion(entryContent, entryId) {
  ...
  const startRegex = new RegExp(
    `^\\s*(?:>\\s*)*${escapeForRegExp(startMarker)}\\s*$`,
    "m"
  );
  const endRegex = new RegExp(
    `^\\s*(?:>\\s*)*${escapeForRegExp(endMarker)}\\s*$`,
    "m"
  );
...
function findEntryRegions(noteContent) {
  ...
  return markers.map((match, index) => {
    ...
    const markerStart = match.index ?? 0;
    const start = starts[index] ?? markerStart;
    ...
    if (markerMeta) {
      ...
      if (!metaMatch?.[1]) {
        return { markerStart, start, end, markerLine: markerText, meta };
      }
      try {
...
function positionToOffset(content, position) {
  ...
  const safeCh = Math.max(0, Math.min(position.ch, lines[safeLine]?.length ?? 0));
  let offset = 0;
  ...
  async insertContent(content, file, position) {
    try {
      ...
      if (speakerLabels.length > 0) {
        ...
        const baseText = transcript.segments[0]?.text || "";
...
${baseText}`.trim();
  ...
  /**
   * Inserts content at the specified position in a file, with idempotency check
   */
  ...
  /**
   * Inserts or updates a live transcription block in a file
   */
  async upsertLiveTranscriptionBlock(file, position, markerId, transcription) {
    ...
    const collapsed = this.getLiveCalloutCollapsedState(file.path) ?? false;
  ...
  /**
   * Removes a live transcription block from a file
   */
  ...
  /**
   * Removes all live transcription blocks (legacy cleanup)
   */
  ...
  /**
   * Replaces a live transcription block with final formatted content
   */
  async replaceLiveTranscriptionBlockWithFinalContent(file, markerId, finalContent, fallbackPosition) {
    ...
    const entry = this.injectEntryMarkerIntoCallout(
      withMetadata,
      this.buildEntryMarker(markerHash)
    );
...
`;
  ...
  stripAllLegacyLiveBlocks(fileContent) {
    ...
    while (match) {
      ...
      const markerIndex = (match.index ?? 0) - offset;
...
${content}`;
    ...
    if (replacedLine === existingLine) {
      const fallback = existingLine.replace(
        /(>\[![^\]]+\])\s*.*/,
        `$1- ${title}`
      );
...
var RuntimeLogger = class {
  /**
   * Creates a new log context with unique job and correlation IDs
   */
   ...
   * Writes are serialized to prevent file corruption.
   */
...
`;
  ...
  /**
   * Prunes old log entries by age and caps file size.
   */
  static async prune(plugin, options) {
    const maxBytes = options?.maxBytes ?? this.MAX_BYTES_DEFAULT;
    const maxAgeMs = options?.maxAgeMs ?? this.MAX_AGE_MS_DEFAULT;
    this.logWriteChain = this.logWriteChain.then(async () => {
      const adapter = plugin.app.vault.adapter;
      const exists = await adapter.exists(this.LOG_FILE);
      if (!exists)
  ...
  /**
   * Ensures the log directory exists (idempotent, cached after first call)
   */
...
var _JobStore = class {
  ...
  /**
   * Insert or update a job record (matched by jobId)
   */
  async upsertJob(job) {
  ...
  /**
   * Update status (and optional error) of an existing job
   */
  ...
  /**
   * Insert or update a checkpoint (matched by jobId + index)
   */
  ...
  /**
   * Returns all jobs that are queued, running, or failed
   */
  ...
  /**
   * Returns the most recently updated incomplete job matching optional filters
   */
  ...
  /**
   * Returns all checkpoints for a job, sorted by index ascending
   */
  ...
  /**
   * Returns the latest committed checkpoint for a given job and stage
   */
  ...
  /**
   * Prunes old completed/canceled jobs and orphaned checkpoints
   */
  async prune(options) {
    const maxJobs = options?.maxJobs ?? 500;
    const maxCheckpoints = options?.maxCheckpoints ?? 2e3;
    const maxAgeMs = options?.maxAgeMs ?? 14 * 24 * 60 * 60 * 1e3;
    const cutoff = Date.now() - maxAgeMs;
    await this.withState(async (state) => {
  ...
  /**
   * Demotes stale failed jobs to canceled status.
   * Returns the number of jobs changed.
   */
  ...
  /**
   * Serializes state mutations through the write chain
   */
  ...
  /**
   * Writes state to disk as formatted JSON
   */
  ...
  /**
   * Moves a corrupt file aside so it can be inspected later
   */
...
var _LocalQueueBackend = class {
  ...
  /**
   * Adds a new job to the queue and returns it
   */
   ...
   * If preferredJobId is provided, that job is claimed if available.
  ...
  /**
   * Marks a job as completed with an optional result reference
   */
  ...
  /**
   * Marks a job as failed with a reason and optional retry time
   */
  ...
  /**
   * Retries a failed or retry_scheduled job by re-queuing it
   */
  ...
  /**
   * Returns a snapshot of all jobs sorted by most recently updated
   */
  async getSnapshot() {
    const state = await this.readState();
  ...
  /**
   * Prunes old terminal jobs by age and caps total job count
   */
  async prune(options) {
    const maxJobs = options?.maxJobs ?? 500;
    const maxAgeMs = options?.maxAgeMs ?? 14 * 24 * 60 * 60 * 1e3;
  ...
  /**
   * Checks whether a job can be claimed right now
   */
  ...
  /**
   * Validates that the caller holds the lease
   */
  ...
  /**
   * Clears lease fields from a job
   */
  clearLease(job) {
  ...
  /**
   * Generates a unique lease token
   */
  newLeaseToken() {
  ...
  /**
   * Writes queue state to disk as formatted JSON
   */
  async writeState(state) {
    const adapter = this.plugin.app.vault.adapter;
    await this.ensureDir(adapter);
  ...
  /**
   * Moves a corrupt file aside for inspection
   */
  async quarantineCorruptFile(adapter, path, raw) {
    try {
      const quarantinePath = `${path}.corrupt.${Date.now()}.json`;
...
var BatchRoutingPolicy = class {
  ...
  /**
   * Makes a routing decision for a given source file and size
   */
  ...
  /**
   * Detects whether a file was recorded locally or uploaded
   */
...
var _BackendBatchOrchestrationService = class {
  ...
  /**
   * Sends a large audio file to the backend for transcription and
  ...
  /**
   * Uploads the source audio blob to the backend, chunking if necessary
   */
  ...
  /**
   * Polls the backend status endpoint until the job completes, fails, or times out
   */
  async pollForResult(statusUrl, defaultResultUrl, backendJobId, logContext) {
    ...
    while (Date.now() - startedAt <= timeoutMs) {
      ...
      lastUiState = uiState;
      this.plugin.showProcessingStatus(
        `Backend: ${formatUiStateLabel(uiState)}`
      );
      ...
      if (isCompletedStatus(normalized)) {
        ...
        if (!resultUrl) {
          throw new Error(
            "Backend job completed but no result payload was provided."
          );
        }
        ...
        if (!transcript) {
          throw new Error(
            "Backend result payload missing transcription text."
          );
        }
        await RuntimeLogger.log(
          this.plugin,
  ...
  async requestJson(url, method, body) {
    ...
    if (!response.json) {
      throw new Error(
        `Backend returned invalid JSON response for ${method} ${url}`
      );
    }
    return response.json;
  }
  /**
   * Builds request headers, injecting auth token if configured
   */
  buildHeaders(extra) {
    const headers = { ...extra };
...
function classifyError(error) {
  ...
  if (includesAny(message, ["unauthorized", "invalid api key", "api key is not configured", "forbidden", "401", "403"])) {
  ...
  if (includesAny(message, ["invalid request", "invalid response format", "invalid transcription response", "file too large", "payload", "400", "unprocessable"])) {
  ...
  if (includesAny(message, ["network", "failed to fetch", "econn", "enotfound", "socket", "dns", "connection"])) {
    return { errorClass: "network", retryable: true };
  }
  if (includesAny(message, ["500", "502", "503", "504", "server error", "internal server error", "bad gateway"])) {
...
// src/utils/RecordingProcessor.ts
init_StructuredOutputRenderer();
init_TranscriptSchema();
init_MeetingTemplates();
var _RecordingProcessor = class {
  constructor(plugin) {
    ...
    // C3: Synchronous lock to prevent TOCTOU race on processing state
  ...
  static getInstance(plugin) {
    return this.instance ?? (this.instance = new _RecordingProcessor(plugin));
  }
  /**
   * Processes a recording: transcribes audio and inserts the content into the document.
   *
   * Pipeline stages:
   * 1. Audio Processing  - normalize / chunk the raw audio blob
   * 2. Transcription      - route to direct-provider or backend-batch, with retry
   * 3. Content Insertion  - render transcript + post-processing into the note
   *
   * Each stage is checkpointed so the job can be resumed after a crash.
   */
  async processRecording(audioBlob, activeFile, cursorPosition, audioFilePath, enhancementOptions) {
    ...
    let queueJobId = null;
    let leaseToken = null;
    if (this.processingLock || this.processingState.getIsProcessing()) {
    ...
    try {
      ...
      if (enhancementOptions?.useStructuredOutput && result.postProcessing) {
        const template = enhancementOptions.meetingTemplateId ? getBuiltInTemplate(enhancementOptions.meetingTemplateId) : void 0;
        await this.insertStructuredOutput(
          ...
          cursorPosition,
          {
            ...
            durationSeconds: await this.resolveDurationSeconds(audioResult.audioBlob, result.transcription) ?? void 0
          },
          template,
          enhancementOptions?.userNotes
        );
      } else {
  ...
  /**
   * Returns all incomplete (running / failed) jobs from the recovery store.
   */
  async getIncompleteJobs() {
   ...
   * - Demotes stale failed jobs to canceled
   * - Prunes old completed/canceled jobs
   * - Prunes the local queue
   * - Reconciles queue vs recovery states
   */
  ...
  /**
   * Attempts to resume a previously failed job from its last checkpoint.
   * If the transcription_ready checkpoint exists but note_written does not,
   * re-inserts the content into the target note.
   *
   * Returns true if resume succeeded, false if the job cannot be resumed.
   */
  async resumeJob(jobId) {
    ...
    if (!anchorValid) {
      ...
      new import_obsidian5.Notice(
        "NeuroVox recovered transcript inserted at end of note because the original cursor anchor changed."
      );
    }
    await this.documentInserter.insertContent(
      {
   ...
   * Unlike processRecording(), this skips the audio-processing and transcription stages
   * because the transcript was produced by a live/streaming session.
   */
  async processStreamingResult(transcriptionResult, activeFile, cursorPosition, options) {
    ...
    const recoveryJobId = existingStreamJob?.jobId || logContext.jobId;
    const now = new Date().toISOString();
    const resumeAnchor = await this.createResumeAnchor(activeFile, cursorPosition);
    const baseJob = {
      jobId: recoveryJobId,
    ...
    try {
      ...
      let postProcessing;
      const useTemplate = options?.useStructuredOutput && options?.meetingTemplateId;
      if (useTemplate || this.plugin.settings.generatePostProcessing) {
        this.processingState.startStep("Post-processing");
        if (useTemplate) {
          const template = getBuiltInTemplate(options.meetingTemplateId);
          postProcessing = await this.executeWithRetry(
            () => this.generateTemplatePostProcessing(transcriptionResult, template, options?.userNotes)
          );
        } else {
          postProcessing = await this.executeWithRetry(
      ...
      let savedAudioPath = options?.audioFilePath;
      ...
      await RuntimeLogger.log(this.plugin, logContext, "note_render_start", { status: "started" });
      if (options?.useStructuredOutput && postProcessing) {
        const template = options.meetingTemplateId ? getBuiltInTemplate(options.meetingTemplateId) : void 0;
        await this.insertStructuredOutput(
          transcriptionResult,
          postProcessing,
          activeFile,
          cursorPosition,
          {
            ...
            durationSeconds: await this.resolveDurationSeconds(
              options?.audioBlob,
              transcriptionResult,
        ...
        await this.documentInserter.insertContent(
          {
            ...
            postProcessing,
            audioFilePath: savedAudioPath,
            sourceSizeMb: options?.audioBlob ? (options.audioBlob.size / (1024 * 1024)).toFixed(2) : void 0,
            entryTitle: this.buildDurationEntryTitle(
...
${transcription}`;
  ...
  /**
   * Inserts structured meeting output into the note using the StructuredOutputRenderer.
   * Parses AI post-processing into semantic sections and renders them as Obsidian callouts.
   * Uses template section definitions when a template is provided.
   */
  async insertStructuredOutput(transcription, postProcessing, activeFile, cursorPosition, meta, template, userNotes) {
    const renderer = new StructuredOutputRenderer();
    const transcript = fromPlainTranscription(transcription);
    const sectionDefs = template ? template.sections.map((s) => ({ id: s.id, heading: s.heading, calloutType: s.calloutType })) : DEFAULT_SECTIONS;
    const sections = StructuredOutputRenderer.parseSectionsFromPostProcessing(
      postProcessing,
      sectionDefs
    );
    const speakerLabels = extractSpeakerLabels(transcription);
    let durationStr;
    if (meta.durationSeconds && meta.durationSeconds > 0) {
      const mins = Math.floor(meta.durationSeconds / 60);
      const secs = meta.durationSeconds % 60;
      durationStr = mins > 0 ? `${mins}m ${secs}s` : `${secs}s`;
    }
    const structuredContent = renderer.render({
      transcript,
      sections,
      meta: {
        title: template ? `${template.name} Notes` : void 0,
        date: new Date().toLocaleDateString(),
        duration: durationStr,
        participants: speakerLabels.length > 0 ? speakerLabels : void 0,
        audioFilePath: meta.audioFilePath
      },
      userNotes: userNotes || void 0,
      includeTimestamps: this.plugin.settings.includeTimestamps
    });
    const fileContent = await this.plugin.app.vault.read(activeFile);
    const lines = fileContent.split("\n");
    const offset = lines.slice(0, cursorPosition.line).reduce((acc, line) => acc + line.length + 1, 0) + cursorPosition.ch;
    let finalContent = fileContent.slice(0, offset) + structuredContent + fileContent.slice(offset);
    if (this.plugin.settings.addMeetingFrontmatter && !fileContent.includes("neurovox_entry_id:")) {
      const actionCount = sections.filter((s) => s.id === "actions" || s.id === "followup" || s.id === "nextsteps").length;
      const decisionCount = sections.filter((s) => s.id === "decisions").length;
      const entryId = `nv_${Date.now().toString(36)}`;
      const fmLines = [
        "---",
        "type: meeting",
        `meeting_type: ${template?.id || "general"}`,
        `date: ${new Date().toISOString().split("T")[0]}`,
        ...durationStr ? [`duration: ${durationStr}`] : [],
        ...speakerLabels.length > 0 ? [`participants: [${speakerLabels.join(", ")}]`] : [],
        ...actionCount > 0 ? [`action_items: ${actionCount}`] : [],
        ...decisionCount > 0 ? [`decisions: ${decisionCount}`] : [],
        `neurovox_entry_id: ${entryId}`,
        "---",
        ""
      ].join("\n");
      if (finalContent.startsWith("---\n")) {
        const fmEnd = finalContent.indexOf("\n---\n", 4);
        if (fmEnd !== -1) {
          const existingFm = finalContent.slice(4, fmEnd);
          const newFields = [
            `meeting_type: ${template?.id || "general"}`,
            ...durationStr ? [`duration: ${durationStr}`] : [],
            ...speakerLabels.length > 0 ? [`participants: [${speakerLabels.join(", ")}]`] : [],
...
${newFields}
---
${finalContent.slice(fmEnd + 5)}`;
        }
      } else {
        finalContent = fmLines + finalContent;
      }
    }
    await this.plugin.app.vault.modify(activeFile, finalContent);
  }
  /**
   * Generates post-processing using a meeting template's extraction prompt.
   * For long transcripts, automatically chunks and merges.
   */
  async generateTemplatePostProcessing(transcription, template, userNotes) {
    const adapter = await this.getAdapter(
      this.plugin.settings.postProcessingProvider,
      "language"
    );
    const model = this.plugin.settings.postProcessingModel;
    const maxWords = getMaxTranscriptWords(model);
    const wordCount = transcription.split(/\s+/).length;
    if (wordCount <= maxWords) {
      let userPrompt = template.userPromptTemplate.replace("{transcript}", transcription);
      if (userNotes) {
        userPrompt = `The user took these rough notes during the meeting:
${userNotes}

Using the user's notes as a guide for what was important, produce structured meeting notes. Preserve the user's original phrasing where it adds context the transcript doesn't capture.

${userPrompt}`;
      }
      userPrompt = userPrompt.replace("{user_notes}", userNotes || "");
      return adapter.generateResponse(
        userPrompt,
        model,
        {
    ...
    this.plugin.showProcessingStatus("Long transcript \u2014 processing in chunks");
    const chunks = this.splitTranscriptIntoChunks(transcription, maxWords);
    const chunkResults = [];
    for (let i = 0; i < chunks.length; i++) {
      this.plugin.showProcessingStatus(`Processing chunk ${i + 1}/${chunks.length}`);
      try {
        const chunkPrompt = template.userPromptTemplate.replace("{transcript}", chunks[i]);
        ...
        const reason = error instanceof Error ? error.message : String(error);
        chunkResults.push(`(Chunk ${i + 1} failed: ${reason})`);
      }
    }
    if (chunkResults.length === 1) {
      return chunkResults[0];
    }
    this.plugin.showProcessingStatus("Merging chunk results");
    const mergePrompt = `You previously summarized a long meeting transcript in ${chunkResults.length} segments. Merge these segment summaries into a single cohesive set of meeting notes. Deduplicate action items and decisions. Maintain all section headings.

` + chunkResults.map((r, i) => `--- Segment ${i + 1} ---
${r}`).join("\n\n") + "\n\nMerged output:";
    return adapter.generateResponse(
      mergePrompt,
      model,
  ...
  /**
   * Splits a transcript into chunks of approximately maxWords each,
   * breaking at sentence boundaries to avoid splitting mid-thought.
   */
  splitTranscriptIntoChunks(transcription, maxWords) {
    ...
    while (start < words.length) {
      ...
      if (end < words.length) {
        ...
        if (lastSentenceEnd > segment.length * 0.5) {
          const truncated = segment.slice(0, lastSentenceEnd + 1);
          end = start + truncated.split(/\s+/).length;
        }
      }
  ...
  /**
   * Gets and validates the appropriate AI adapter.
   * If the adapter is not ready, attempts a timed validation.
   */
  async getAdapter(provider, category) {
    const adapter = this.plugin.aiAdapters.get(provider);
    if (!adapter) {
  ...
  /**
   * Validates an adapter's API key with a timeout guard.
   */
  async validateAdapterWithTimeout(adapter) {
   ...
   * Uses classifyError() to determine whether an error is retryable,
   * and applies exponential back-off with error-class-specific multipliers.
   */
  ...
  /**
   * Computes exponential back-off delay with error-class-specific multipliers.
  ...
  /**
   * Handles error display via Obsidian Notice.
   */
  ...
  /**
   * Resolves the recording duration in seconds from the best available source:
   * 1. Explicit preferredSeconds (from live timer)
   * 2. Decode audio blob via Web Audio API
   * 3. Parse [HH:MM:SS] timestamp tokens from the transcript
   */
  ...
  /**
   * Estimates audio duration by decoding the blob via Web Audio API.
   */
  ...
  /**
   * Scans the transcript for [HH:MM:SS] timestamp tokens and returns the
   * maximum time found, or null if no tokens are present.
   */
  ...
  /**
   * Builds a human-readable duration label (e.g. "Transcription - 02:34")
   * from a duration in seconds. Returns null if duration is invalid.
   */
  ...
  /**
   * Reconciles local queue state with recovery job state on startup.
   * If a batch job is marked as running but its queue entry is missing or failed,
   * the recovery job is moved to failed.
   */
  ...
  /**
   * Creates a resume anchor capturing the cursor position and surrounding content hash.
   */
  ...
  /**
   * Checks whether the resume anchor is still valid by comparing
   * the stored context hash with the current content around the insertion line.
   */
  ...
  /**
   * Computes a FNV-1a-style hash of the 5 lines surrounding `line` in `content`.
   * Used for resume anchor validation.
   */
  ...
  render() {
    const openaiSetting = new import_obsidian8.Setting(this.contentEl).setName("OpenAI API Key").setDesc("Enter your OpenAI API key").addText((text) => {
      ...
      text.onChange(async (value) => {
        ...
        const isValid = (await adapter.validateApiKey()).valid;
        if (isValid) {
...
var PostProcessingAccordion = class extends BaseAccordion {
  ...
  async updateMaxTokensLimit(modelId) {
    ...
    const maxTokens = model?.maxTokens || 1e3;
...
// src/utils/KeepAliveManager.ts
var KeepAliveManager = class {
  constructor(onSuspension) {
    this.onSuspension = onSuspension;
    this.wakeLockSentinel = null;
    // WakeLockSentinel type not in TS lib
    ...
    this.onSuspensionDetected = null;
    this.handleVisibilityChange = () => {
      if (document.visibilityState === "visible") {
        const gap = Date.now() - this.lastHeartbeat;
        if (gap > 3e4 && this.onSuspensionDetected) {
          console.warn(`NeuroVox: Suspension detected \u2014 heartbeat gap ${Math.round(gap / 1e3)}s`);
          this.onSuspensionDetected();
        }
        if (this.activeMethod === "wakelock") {
          this.reacquireWakeLock();
        }
      }
    };
    this.onSuspensionDetected = onSuspension || null;
  }
  async activate() {
    if ("wakeLock" in navigator) {
      try {
        this.wakeLockSentinel = await navigator.wakeLock.request("screen");
        this.activeMethod = "wakelock";
        this.startHeartbeat();
        this.registerVisibilityHandler();
        console.log("NeuroVox: WakeLock acquired");
        return "wakelock";
      } catch (e) {
        console.warn("NeuroVox: WakeLock request failed:", e);
      }
    }
    try {
      ...
      this.activeMethod = "audio_keepalive";
      this.startHeartbeat();
      this.registerVisibilityHandler();
      console.log("NeuroVox: AudioContext keepalive activated");
      return "audio_keepalive";
    } catch (e) {
      console.warn("NeuroVox: AudioContext keepalive failed:", e);
    }
    this.activeMethod = "none";
    this.startHeartbeat();
    this.registerVisibilityHandler();
    console.warn("NeuroVox: No keep-alive method available");
    return "none";
  }
  deactivate() {
    if (this.wakeLockSentinel) {
      try {
        ...
        console.warn("NeuroVox: Error releasing WakeLock:", e);
      }
      this.wakeLockSentinel = null;
    }
    if (this.oscillator) {
    ...
    if (this.audioContext) {
      try {
        ...
        console.warn("NeuroVox: Error closing AudioContext:", e);
      }
      this.audioContext = null;
    }
    ...
    this.activeMethod = "none";
    this.lastHeartbeat = 0;
    console.log("NeuroVox: KeepAlive deactivated");
  }
  getActiveMethod() {
  ...
  async reacquireWakeLock() {
    ...
    try {
      this.wakeLockSentinel = await navigator.wakeLock.request("screen");
      console.log("NeuroVox: WakeLock re-acquired after visibility change");
    } catch (e) {
      console.warn("NeuroVox: Failed to re-acquire WakeLock:", e);
    }
  }
};

// src/utils/RecordingManager.ts
var _AudioRecordingManager = class {
  constructor(plugin, lifecycleCallbacks) {
    this.plugin = plugin;
    this.recorder = null;
    this.stream = null;
  ...
  getAudioConfig() {
    ...
    return {
      type: "audio",
      mimeType: "audio/webm",
      // Use WebM container for better compression
  ...
  async initialize() {
    try {
      ...
      if (error instanceof DOMException || error && typeof error.name === "string") {
  ...
  start(options) {
    ...
    const streamingMimeType = options?.timeSlice ? this.getPreferredStreamingMimeType() : void 0;
    const config = {
      ...this.getAudioConfig(),
      // MediaStreamRecorder emits timeslice/final chunks more reliably for streaming mode.
      recorderType: options?.timeSlice ? import_recordrtc.default.MediaStreamRecorder : import_recordrtc.default.StereoAudioRecorder,
      ...
      ondataavailable: options?.onDataAvailable ? async (blob) => {
        if (!blob || blob.size === 0)
          return;
        const task = Promise.resolve(options.onDataAvailable(blob)).catch((e) => console.warn("NeuroVox:", e)).finally(() => {
    ...
    this.keepAliveManager = new KeepAliveManager(() => {
      this.lifecycleCallbacks.onLifecycleInterrupt?.("keep_alive_suspension");
    });
    this.keepAliveManager.activate().catch(
      (e) => console.warn("NeuroVox: KeepAliveManager activation failed:", e)
    );
  }
  ...
  async stop() {
    ...
    return new Promise((resolve) => {
      ...
      let settled = false;
      const settleWith = (blob) => {
        if (settled)
          return;
        settled = true;
        if (!blob || blob.size === 0) {
          new import_obsidian12.Notice("Recording may not have saved properly. Please try again.");
          resolve(blob ?? new Blob([], { type: "audio/webm" }));
          return;
        }
      ...
      const timeout = window.setTimeout(() => {
        console.warn("NeuroVox: stopRecording timed out");
        const blob = this.recorder?.getBlob() || null;
        settleWith(blob);
      }, _AudioRecordingManager.STOP_TIMEOUT_MS);
  ...
  registerLifecycleHandlers() {
    ...
    this.onVisibilityChangeBound = () => {
      if (document.visibilityState === "hidden") {
        ...
        this.lifecycleCallbacks.onLifecycleInterrupt?.("app_hidden");
      } else if (document.visibilityState === "visible" && this.lifecycleInterrupted) {
        this.lifecycleInterrupted = false;
        this.lifecycleCallbacks.onLifecycleResume?.();
      }
    };
    document.addEventListener("visibilitychange", this.onVisibilityChangeBound);
    this.onPageHideBound = () => {
      this.lifecycleInterrupted = true;
      this.lifecycleCallbacks.onLifecycleInterrupt?.("page_hide");
    };
  ...
  deactivateKeepAlive() {
    if (this.keepAliveManager) {
      this.keepAliveManager.deactivate();
      this.keepAliveManager = null;
    }
  }
  getKeepAliveMethod() {
    return this.keepAliveManager?.getActiveMethod() ?? "none";
  }
  getState() {
...
var AudioRecordingManager = _AudioRecordingManager;
AudioRecordingManager.STOP_TIMEOUT_MS = 1e4;
...
// src/utils/transcription/DeepgramLiveAdapter.ts
var _DeepgramLiveAdapter = class {
  ...
  async sendAudio(chunk) {
    ...
    if (this.ws && this.ws.bufferedAmount > _DeepgramLiveAdapter.BACKPRESSURE_THRESHOLD) {
      console.warn(
        `NeuroVox: WebSocket backpressure \u2013 bufferedAmount=${this.ws.bufferedAmount}, skipping frame (${frame.byteLength} bytes)`
      );
      return;
    }
    this.ws?.send(frame);
  ...
  async stop() {
    ...
    await new Promise((resolve) => {
      ...
      const timeout = window.setTimeout(() => {
        try {
          this.ws?.close(1e3, "client_stop_timeout");
        } catch (e) {
        }
        done();
  ...
  handleMessage(rawData) {
    try {
      ...
      if (!text)
        return;
      const startedAt = typeof payload?.start === "number" ? payload.start : void 0;
      const speakerTurns = this.extractSpeakerTurns(alt);
      const isFinal = payload?.is_final === true || payload?.speech_final === true;
      if (isFinal) {
        this.listener?.({ type: "final", text, startedAtSec: startedAt, speakerTurns });
      } else {
        this.listener?.({ type: "interim", text, startedAtSec: startedAt, speakerTurns });
      }
    } catch (error) {
      this.listener?.({
        type: "error",
  ...
  extractSpeakerTurns(alternative) {
    ...
    for (const word of words) {
      const punctWord = typeof word?.punctuated_word === "string" ? word.punctuated_word : typeof word?.word === "string" ? word.word : "";
      if (!punctWord.trim())
        continue;
      const speakerRaw = typeof word?.speaker === "number" ? word.speaker : void 0;
      const speakerId = typeof speakerRaw === "number" ? speakerRaw + 1 : void 0;
      const wordStart = typeof word?.start === "number" ? word.start : void 0;
      if (currentWords.length === 0) {
...
DeepgramLiveAdapter.BACKPRESSURE_THRESHOLD = 1048576;

// src/utils/transcription/WebAudioPcmSource.ts
...
var _StreamingTranscriptionService = class {
  constructor(plugin, callbacks) {
    ...
    // Transport profile (for 400-error fallback)
    this.transportProfile = "native";
    this.consecutive400Errors = 0;
    // Live (Deepgram WebSocket) state
  ...
  /**
   * Computes a dynamic finalize timeout based on how long the recording lasted.
   * Formula: 2x recording duration + 30s buffer, clamped between 1 min and 5 min.
   */
  computeFinalizeTimeout(recordingDurationMs) {
    return Math.min(
      Math.max(recordingDurationMs * 2 + 3e4, _StreamingTranscriptionService.FINALIZE_TIMEOUT_MIN_MS),
      _StreamingTranscriptionService.FINALIZE_TIMEOUT_MAX_MS
    );
  }
  ...
  async startLiveSession(stream) {
    ...
    if (this.liveStarted)
      return;
    this.plugin.settings.saveLiveRecordingAudio = true;
    ...
    await this.liveAudioSource.start(async (frame) => {
      ...
      try {
        await this.liveAdapter?.sendAudio(frame);
  ...
  pauseLive() {
    ...
    this.liveAudioSource?.pause();
  }
  resumeLive() {
    if (this.transportMode !== "deepgram_ws")
      return;
    this.livePaused = false;
    if (this.liveReconnectRequired || !this.liveAdapter?.isOpen()) {
      this.liveReconnectRequired = false;
      const reconnectPromise = this.ensureLiveAdapterStarted().catch(async (error) => {
        const reason = error instanceof Error ? error.message : String(error);
      ...
      this.liveAdapterStartPromise = reconnectPromise;
    }
    this.liveAudioSource?.resume();
    const rejectReason = this.chunkQueue.getLastRejectReason();
    if (rejectReason) {
      new import_obsidian15.Notice("Some audio may have been lost during pause. Recording continues.");
      void RuntimeLogger.log(this.plugin, this.logContext, "backpressure_data_loss", {
        status: "warning",
        reason: rejectReason,
        mode: this.transportMode
      });
      this.chunkQueue.resetLastRejectReason();
    }
  }
  // ── Chunk-queue transport ───────────────────────────────────────
  ...
  async processChunkWithRetry(chunk, metadata) {
    ...
    while (attempt <= _StreamingTranscriptionService.CHUNK_MAX_RETRIES) {
      try {
        await Promise.race([
          this.processChunkOnce(chunk, metadata),
          this.sleep(_StreamingTranscriptionService.CHUNK_PROCESSING_TIMEOUT_MS).then(() => {
            throw new Error(`Chunk ${metadata.id} processing timed out after ${_StreamingTranscriptionService.CHUNK_PROCESSING_TIMEOUT_MS}ms`);
          })
        ]);
        this.consecutive400Errors = 0;
        return;
  ...
  async finishProcessing() {
    if (this.isFinalized && this.finalizedResult !== null) {
      await RuntimeLogger.log(this.plugin, this.logContext, "finalize_skipped", {
        status: "skipped",
    ...
    if (this.transportMode === "deepgram_ws") {
      ...
      this.finalizedResult = this.buildFinalResult();
      return this.finalizedResult;
    }
    if (this.processingPromise) {
      const recordingDurationMs = Date.now() - this.liveSessionStartTs;
      const finalizeTimeoutMs = this.computeFinalizeTimeout(recordingDurationMs);
      try {
        await Promise.race([
          this.processingPromise,
          this.sleep(finalizeTimeoutMs).then(() => {
            throw new Error(`Finalize timed out after ${Math.round(finalizeTimeoutMs / 1e3)}s while waiting for queue drain`);
          })
        ]);
      } catch (error) {
  ...
  getStats() {
    return {
      ...
      liveAudioStats: this.liveAudioSource?.getStats() || null
    };
  }
  abort(reason = "manual_abort") {
    this.acceptingChunks = false;
  ...
  async ensureRecoveryJob() {
    if (this.recoveryJobInitialized)
      return;
    const activeView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian15.MarkdownView);
  ...
  // ── Live transcript commit helpers ───────────────────────────────
  ...
  // ── Live transport finalization ─────────────────────────────────
  async finishLiveTransport() {
    if (!this.liveAdapter)
      return;
    try {
      await this.flushBufferedLiveFinals();
      if (this.liveAudioSource?.isRunning()) {
...
// 12 minutes (legacy fallback)
StreamingTranscriptionService.FINALIZE_TIMEOUT_MAX_MS = 3e5;
// 5 minutes cap
StreamingTranscriptionService.FINALIZE_TIMEOUT_MIN_MS = 6e4;
// 1 minute floor
StreamingTranscriptionService.FOUR_HUNDRED_FALLBACK_THRESHOLD = 2;
StreamingTranscriptionService.CHUNK_PROCESSING_TIMEOUT_MS = 6e4;
// 60 seconds
...
// src/utils/document/NoteSnapshotManager.ts
var import_obsidian16 = require("obsidian");
var NoteSnapshotManager = class {
  constructor(plugin) {
    this.plugin = plugin;
    this.startSnapshot = null;
  }
  /**
   * Captures a snapshot of the current note content.
   * Uses editor.getValue() (unsaved buffer) with vault.read() as fallback.
   */
  async captureSnapshot(file) {
    let content;
  ...
  /**
   * Captures and stores the start-of-recording snapshot.
   */
  async captureStartSnapshot(file) {
    this.startSnapshot = await this.captureSnapshot(file);
  }
  /**
   * Captures end-of-recording snapshot and computes user notes added during the meeting.
   * Returns the net-new lines (lines present in end but absent from start).
   * Returns null if no start snapshot was captured.
   */
  async computeUserNotes(file) {
    if (!this.startSnapshot || this.startSnapshot.filePath !== file.path) {
      return null;
    }
    const endSnapshot = await this.captureSnapshot(file);
    const userNoteLines = diffNewLines(this.startSnapshot.content, endSnapshot.content);
    if (userNoteLines.length === 0) {
      return null;
    }
    return userNoteLines.join("\n");
  }
  /**
   * Returns the stored start snapshot (for job metadata persistence).
   */
  getStartSnapshot() {
...
var _InlineRecorderPanel = class {
  constructor(options) {
    ...
    this.enhancementContainerEl = null;
    this.enhancingContainerEl = null;
    ...
    // Pending enhancement state — holds data between recording stop and user choice
    this.pendingTranscript = null;
    this.pendingAudioBlob = null;
    this.pendingAudioPath = null;
    this.pendingDurationSeconds = 0;
    this.pendingUserNotes = null;
    ...
    this.recordingManager = new AudioRecordingManager(this.plugin, {
      onLifecycleInterrupt: (reason) => this.handleLifecycleInterruption(reason),
      onLifecycleResume: () => this.handleLifecycleResume()
    });
    this.documentInserter = new DocumentInserter(this.plugin);
    this.snapshotManager = new NoteSnapshotManager(this.plugin);
  ...
  async start() {
    try {
      ...
      this.timerSeconds = 0;
      this.ui?.updateTimer(this.timerSeconds, Number.POSITIVE_INFINITY, 60);
      this.setState("ready");
    } catch (error) {
  ...
  async startRecordingSession() {
    ...
    try {
      ...
      await this.snapshotManager.captureStartSnapshot(this.activeFile);
      this.timerSeconds = 0;
      this.ui?.updateTimer(this.timerSeconds, Number.POSITIVE_INFINITY, 60);
      this.startTimer();
      this.setState("recording");
      this.plugin.showRecordingIndicator();
  ...
  async stop() {
    ...
    this.stopTimer();
    this.plugin.hideRecordingIndicator();
    ...
    try {
      ...
      this.pendingUserNotes = await this.snapshotManager.computeUserNotes(this.activeFile);
      this.pendingTranscript = transcript;
      this.pendingAudioBlob = this.saveAudioForSession ? finalBlob : null;
      this.pendingDurationSeconds = this.timerSeconds;
      this.setState("pending_enhancement");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  /**
   * User chose "Enhance Notes" — run post-processing through structured output pipeline,
   * then insert the structured meeting note into the document.
   */
  async handleEnhance() {
    if (this.isDisposed || this.state !== "pending_enhancement")
      return;
    this.setState("enhancing");
    try {
      if (this.useStreaming && this.pendingTranscript) {
        await this.plugin.recordingProcessor.processStreamingResult(
          this.pendingTranscript,
          this.activeFile,
          this.cursorPosition,
          {
            audioBlob: this.pendingAudioBlob || void 0,
            durationSeconds: this.pendingDurationSeconds,
            useStructuredOutput: true,
            meetingTemplateId: this.selectedTemplateId,
            userNotes: this.pendingUserNotes || void 0
          }
        );
      } else if (this.pendingAudioBlob) {
        ...
        throw new Error("No transcript or audio available for enhancement");
      }
      this.dispose();
    } catch (error) {
      this.handleFailure("Enhancement failed", error);
    }
  }
  /**
   * User chose "Skip enhancement" — insert raw transcript in the old callout format.
   */
  async handleSkipEnhancement() {
    if (this.isDisposed || this.state !== "pending_enhancement")
      return;
    this.setState("enhancing");
    try {
      if (this.useStreaming && this.pendingTranscript) {
        ...
        throw new Error("No transcript or audio available");
      }
      this.dispose();
    } catch (error) {
      this.handleFailure("Failed to insert transcript", error);
    }
  }
  ...
  createPanel(anchor) {
    ...
    this.enhancementContainerEl = this.uiContainerEl.createDiv({
      cls: "neurovox-inline-recorder-section neurovox-inline-recorder-section-enhancement"
    });
    this.createEnhancementUI(this.enhancementContainerEl);
    this.enhancingContainerEl = this.uiContainerEl.createDiv({
      cls: "neurovox-inline-recorder-section neurovox-inline-recorder-section-enhancing"
    });
    this.createEnhancingUI(this.enhancingContainerEl);
  ...
  createOptionsRow(container) {
    ...
    checkbox.addEventListener("change", () => {
      if (!canEditSaveAudio(this.state)) {
        checkbox.checked = this.saveAudioForSession;
        new import_obsidian17.Notice("Save audio can only be changed before recording starts.");
        return;
      }
      this.saveAudioForSession = checkbox.checked;
      this.plugin.settings.saveLiveRecordingAudio = checkbox.checked;
      void this.plugin.saveSettings({ refreshUi: false, triggerFloatingRefresh: false }).catch((e) => console.warn("NeuroVox:", e));
    });
  ...
  async handleUploadAudio() {
    if (!canUploadAudio(this.state)) {
      new import_obsidian17.Notice("Upload audio is available before recording starts.");
      return;
    }
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        new import_obsidian17.Notice(`Transcribed uploaded audio: ${file.name}`);
        this.dispose();
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        new import_obsidian17.Notice(`Failed to transcribe uploaded audio: ${message}`);
        this.setState("ready");
      }
    };
    input.click();
  ...
  async cancelIncompleteJobs() {
    try {
      ...
      if (pending.length === 0) {
        new import_obsidian17.Notice("No incomplete jobs to cancel.");
        return;
      }
      await Promise.all(pending.map((job) => this.plugin.recordingProcessor.cancelJob(job.jobId)));
      new import_obsidian17.Notice(`Canceled ${pending.length} incomplete job(s).`);
    } catch (e) {
      new import_obsidian17.Notice("Failed to cancel incomplete jobs.");
    }
  }
  createEnhancementUI(container) {
    const templateRow = container.createDiv({ cls: "neurovox-enhancement-template-row" });
    templateRow.createSpan({
      cls: "neurovox-enhancement-template-label",
      text: "Meeting type:"
    });
    const select = templateRow.createEl("select", {
      cls: "neurovox-enhancement-template-select dropdown"
    });
    const defaultId = this.plugin.settings.defaultMeetingTemplate || "general";
    ...
    this.selectedTemplateId = defaultId;
    select.addEventListener("change", () => {
      this.selectedTemplateId = select.value;
    });
    const enhanceBtn = container.createEl("button", {
      cls: "neurovox-enhancement-action mod-cta",
      text: "Enhance Notes"
    });
    enhanceBtn.setAttribute("aria-label", "Enhance meeting notes with AI");
    enhanceBtn.addEventListener("click", () => {
      void this.handleEnhance();
    });
    const skipLink = container.createEl("a", {
      cls: "neurovox-enhancement-skip",
      text: "Skip enhancement"
    });
    skipLink.addEventListener("click", (e) => {
      e.preventDefault();
      void this.handleSkipEnhancement();
    });
  }
  createEnhancingUI(container) {
    this.progressTextEl = container.createEl("span", {
      cls: "neurovox-enhancing-text",
      text: "Enhancing..."
    });
    this.progressTextEl.setAttribute("aria-live", "polite");
    const track = container.createDiv({ cls: "neurovox-enhancing-progress-track" });
    this.progressBarEl = track.createDiv({ cls: "neurovox-enhancing-progress-fill" });
  ...
  /**
   * Sets the progress bar to indeterminate mode (spinner animation).
   */
  setProgressIndeterminate() {
    ...
    this.progressBarEl.removeAttribute("aria-valuemax");
    this.progressBarEl.style.width = "";
    if (this.progressTextEl) {
      this.progressTextEl.setText("Enhancing...");
    }
  }
  /**
   * Sets the progress bar to determinate mode with segment tracking.
   */
  setProgressDeterminate(current, total) {
    if (!this.progressBarEl)
      return;
    this.progressBarEl.removeClass("is-indeterminate");
    const pct = Math.round(current / total * 100);
    this.progressBarEl.style.width = `${pct}%`;
    this.progressBarEl.setAttribute("aria-valuenow", String(current));
    this.progressBarEl.setAttribute("aria-valuemax", String(total));
    if (this.progressTextEl) {
      this.progressTextEl.setText(`Segment ${current}/${total}`);
    }
  }
  positionPanel(panelEl, anchor) {
    ...
    if (this.isMobileSheet) {
      const width = Math.min(280, Math.max(260, parentRect.width - margin * 2));
  ...
  setState(state) {
    ...
    const statusLabels = {
      ...
      pending_enhancement: "Recording complete",
      enhancing: "Enhancing..."
    };
    if (this.statusEl) {
      this.statusEl.setText(statusLabels[state] || state);
    }
    if (this.ui) {
      this.ui.updateState(
  ...
  syncControlAvailability() {
    ...
    const isPendingEnhancement = this.state === "pending_enhancement";
    const isEnhancing = this.state === "enhancing";
    this.readyContainerEl?.toggleClass("is-hidden", !isReady);
    this.activeContainerEl?.toggleClass("is-hidden", !isActive);
    this.finalizingContainerEl?.toggleClass("is-hidden", !isFinalizing);
    this.enhancementContainerEl?.toggleClass("is-hidden", !isPendingEnhancement);
    this.enhancingContainerEl?.toggleClass("is-hidden", !isEnhancing);
    if (this.timerEl) {
    ...
    if (this.cancelJobsButtonEl) {
      this.cancelJobsButtonEl.disabled = isFinalizing || isEnhancing;
    }
  }
  startTimer() {
  ...
  async enqueueLivePreviewUpdate(partialResult) {
    ...
    }).catch((e) => console.warn("NeuroVox:", e));
    await this.livePreviewWriteChain;
  }
  async clearLivePreviewBlock(markerIdOverride) {
    const markerId = markerIdOverride ?? this.livePreviewMarkerId;
    if (!markerId)
      return;
  ...
  handleFailure(message, error) {
    ...
    new import_obsidian17.Notice(`${message}: ${detail}`);
    this.dispose();
  }
  formatTimer(seconds) {
    const minutes = Math.floor(seconds / 60).toString().padStart(2, "0");
...
var UploadBottomSheet = class {
  ...
  open() {
    ...
    this.sheetEl.setAttribute("aria-label", "Upload audio file");
    ...
    (0, import_obsidian18.setIcon)(pickerIcon, "upload");
    ...
    (0, import_obsidian18.setIcon)(fileIcon, "music");
    ...
    (0, import_obsidian18.setIcon)(fileCheck, "check-circle");
    ...
    (0, import_obsidian18.setIcon)(saveIcon, "save");
    ...
    (0, import_obsidian18.setIcon)(sparklesIcon, "sparkles");
...
var _MobileDockPill = class {
  constructor(plugin) {
    ...
    this.saveBtnEl = null;
    this.enhancementEl = null;
    this.enhancingEl = null;
    this.timerSeconds = 0;
    this.timerId = null;
    ...
    this.plugin = plugin;
    this.deviceDetection = DeviceDetection.getInstance();
    this.snapshotManager = new NoteSnapshotManager(plugin);
  ...
  createElement() {
    ...
    (0, import_obsidian19.setIcon)(this.idleIconEl, "mic");
    ...
    const uploadBtn = this.makeIconBtn("upload", "neurovox-dock-pill__icon neurovox-dock-pill__upload", "Upload audio");
    uploadBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      this.handleUploadTap();
    });
    this.expandedEl.appendChild(uploadBtn);
    this.saveBtnEl = this.makeIconBtn("save", "neurovox-dock-pill__icon neurovox-dock-pill__save", "Toggle save audio");
    if (this.saveAudioOn)
    ...
    const micBtn = this.makeIconBtn("mic", "neurovox-dock-pill__icon neurovox-dock-pill__mic", "Start recording");
    micBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      this.handleMicTap();
    });
    this.expandedEl.appendChild(micBtn);
    const closeBtn1 = this.makeIconBtn("x", "neurovox-dock-pill__icon neurovox-dock-pill__close", "Close");
    ...
    this.pauseBtnEl = this.makeIconBtn("pause", "neurovox-dock-pill__pause-btn", "Pause recording");
    ...
    (0, import_obsidian19.setIcon)(stopBtn, "square");
    stopBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      this.handleStopTap();
    });
    this.recordingEl.appendChild(stopBtn);
    const closeBtn2 = this.makeIconBtn("x", "neurovox-dock-pill__icon neurovox-dock-pill__close", "Cancel recording");
    ...
    (0, import_obsidian19.setIcon)(loaderIcon, "loader");
    ...
    const closeBtn3 = this.makeIconBtn("x", "neurovox-dock-pill__icon neurovox-dock-pill__close", "Cancel");
    ...
    this.enhancementEl = document.createElement("div");
    this.enhancementEl.classList.add("neurovox-dock-pill__enhancement");
    const templateSelect = document.createElement("select");
    templateSelect.classList.add("neurovox-dock-pill__template-select", "dropdown");
    const defaultId = this.plugin.settings.defaultMeetingTemplate || "general";
    ...
    this.enhancementEl.appendChild(templateSelect);
    const enhanceBtn = document.createElement("button");
    enhanceBtn.classList.add("neurovox-dock-pill__enhance-btn", "mod-cta");
    enhanceBtn.textContent = "Enhance";
    enhanceBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      void this.handleEnhance();
    });
    this.enhancementEl.appendChild(enhanceBtn);
    const skipBtn = document.createElement("a");
    skipBtn.classList.add("neurovox-dock-pill__skip-btn");
    ...
    this.enhancementEl.appendChild(skipBtn);
    this.contentEl.appendChild(this.enhancementEl);
    this.enhancingEl = document.createElement("div");
    this.enhancingEl.classList.add("neurovox-dock-pill__enhancing");
    const enhancingText = document.createElement("span");
    enhancingText.classList.add("neurovox-dock-pill__status-text");
    enhancingText.textContent = "Enhancing...";
    this.enhancingEl.appendChild(enhancingText);
    this.contentEl.appendChild(this.enhancingEl);
  ...
  makeIconBtn(iconName, cls, ariaLabel) {
    ...
    (0, import_obsidian19.setIcon)(btn, iconName);
  ...
  handleUploadTap() {
    if (this.state !== "expanded")
      return;
    const activeView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian19.MarkdownView);
    if (!activeView || !activeView.file) {
      new import_obsidian19.Notice("No active note found to insert transcription.");
      return;
    }
  ...
  async processUploadedFile(file) {
    try {
      ...
      new import_obsidian19.Notice(`Transcribed uploaded audio: ${file.name}`);
      this.setState("idle");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian19.Notice(`Failed to transcribe: ${message}`);
      this.setState("idle");
    }
  }
  handleSaveTap() {
  ...
  async startRecordingSession() {
    ...
    try {
      ...
      if (this.useStreaming && !this.streamingService) {
        this.streamingService = new StreamingTranscriptionService(this.plugin, {
          onMemoryWarning: (usage) => {
            new import_obsidian19.Notice(`Memory usage high: ${Math.round(usage)}%`);
          },
          onChunkCommitted: async (_chunkText, _metadata, partialResult) => {
            if (!this.plugin.settings.showLiveChunkPreviewInNote)
              return;
      ...
      if (this.useStreaming && this.streamingService) {
        ...
        if (!stream)
          throw new Error("Microphone stream unavailable");
        await this.streamingService.startLiveSession(stream);
        this.saveAudioOn = this.plugin.settings.saveLiveRecordingAudio;
      ...
      this.setState("recording");
      this.plugin.showRecordingIndicator();
      new import_obsidian19.Notice("Recording started");
    } catch (error) {
      this.handleFailure("Failed to start recording", error);
    }
  }
  handlePauseTap() {
    ...
    try {
      if (this.state === "paused") {
        ...
        this.startTimer();
        this.setState("recording");
        (0, import_obsidian19.setIcon)(this.pauseBtnEl, "pause");
      } else {
        if (this.useStreaming && this.streamingService) {
          this.streamingService.pauseLive();
          if (this.liveAudioCaptureActive)
        ...
        this.stopTimer();
        this.setState("paused");
        (0, import_obsidian19.setIcon)(this.pauseBtnEl, "play");
      }
    } catch (error) {
  ...
  async handleStopTap() {
    ...
    try {
      ...
      this.pendingTranscript = transcript;
      this.pendingAudioBlob = this.saveAudioOn ? finalBlob : null;
      this.pendingDurationSeconds = this.timerSeconds;
      this.setState("pending_enhancement");
    } catch (error) {
      if (this.streamingService) {
  ...
  async handleEnhance() {
    if (this.state !== "pending_enhancement")
      return;
    this.setState("enhancing");
    try {
      if (this.useStreaming && this.pendingTranscript) {
  ...
  handleLifecycleInterruption(_reason) {
    if (this.state !== "recording")
      return;
    this.handlePauseTap();
    new import_obsidian19.Notice("Recording paused due to app interruption. Resume when back.");
  }
  handleLifecycleResume() {
    if (this.state !== "paused")
      return;
  ...
  async stopRecorderWithTimeout() {
    return await Promise.race([
      this.recordingManager.stop(),
      new Promise(
        (_, reject) => window.setTimeout(() => reject(new Error("Recorder stop timed out")), _MobileDockPill.RECORDER_STOP_TIMEOUT_MS)
      )
    ]);
  }
  handleCloseTap() {
    if (this.state === "expanded") {
      ...
      this.setState("idle");
    } else if (this.state === "finalizing" || this.state === "enhancing") {
      this.cancelTranscription();
      this.setState("idle");
    } else if (this.state === "pending_enhancement") {
      void this.handleSkipEnhancement();
    }
  }
  cancelRecording() {
  ...
  handleFailure(message, error) {
    const detail = error instanceof Error ? error.message : String(error);
    new import_obsidian19.Notice(`${message}: ${detail}`);
    this.resetRecordingState();
    this.setState("idle");
  }
  ...
  updateVisibilityForOverlays() {
    if (!this.containerEl)
      return;
    const hasOverlay = document.querySelector(".modal-container") !== null || document.querySelector(".menu") !== null || document.querySelector(".prompt") !== null;
    const hasSidebar = document.querySelector(".workspace-drawer.is-open") !== null || document.querySelector(".mod-left-split.is-open") !== null || document.querySelector(".mod-right-split.is-open") !== null || document.body.classList.contains("is-left-sidedock-open") || document.body.classList.contains("is-right-sidedock-open");
    if (hasOverlay || hasSidebar) {
  ...
  startDockTracking() {
    ...
    this.dockResizeObserver.observe(this.dockEl);
    this.boundResizeHandler = () => this.updateDockPosition();
  ...
  measureAndPositionAboveDock() {
    if (!this.containerEl)
      return;
    const dockEl = document.querySelector(".mobile-navbar") || document.querySelector(".workspace-tab-header-container-inner") || document.querySelector(".mod-mobile .workspace-tab-header-container");
    if (dockEl) {
...
// src/ui/FloatingButton.ts
var DropReviewModal = class extends import_obsidian20.Modal {
...
var _FloatingButton = class {
  ...
  isInvalid() {
    if (this.isMobileDevice && this.mobilePill) {
      return !this.mobilePill.containerEl?.isConnected;
    }
  ...
  createButton() {
    ...
    (0, import_obsidian20.setIcon)(this.buttonEl, "mic");
    this.buttonEl.addEventListener("click", (event) => {
      if (this.positionManager?.isDragging) {
  ...
  registerMicDropEvents() {
    ...
    this.onMicDragOverBound = (event) => {
      if (!this.hasAudioFile(event.dataTransfer))
        return;
      event.preventDefault();
      this.buttonEl?.addClass("drag-over");
    };
    this.onMicDropBound = (event) => {
      event.preventDefault();
      this.buttonEl?.removeClass("drag-over");
  ...
  handlePositionChange(x, y) {
    ...
    requestAnimationFrame(() => {
      ...
      this.inlineRecorderPanel?.updateAnchor({ x, y });
    });
  }
  ...
  attachToActiveLeaf() {
    if (this.isMobileDevice && this.mobilePill) {
      const activeLeaf2 = this.plugin.app.workspace.getActiveViewOfType(import_obsidian20.MarkdownView);
      if (!activeLeaf2)
        return;
    ...
    const activeLeaf = this.plugin.app.workspace.getActiveViewOfType(import_obsidian20.MarkdownView);
    if (!activeLeaf)
      return;
  ...
  registerPageDropEvents() {
    ...
    this.onPageDragOverBound = (event) => {
      if (!this.hasAudioFile(event.dataTransfer))
        return;
      event.preventDefault();
      this.activeLeafContainer?.addClass("neurovox-drop-target-active");
    };
  ...
  remove() {
    ...
    this.cleanup();
    this.inlineRecorderPanel?.dispose();
    this.inlineRecorderPanel = null;
    if (this.activeLeafContainer) {
      if (this.onPageDragOverBound) {
        this.activeLeafContainer.removeEventListener("dragover", this.onPageDragOverBound);
  ...
  async startDirectRecording() {
    try {
      ...
      new import_obsidian20.Notice("Recording started");
    } catch (error) {
      new import_obsidian20.Notice("Failed to start recording");
      this.cleanup();
    }
  }
  async stopDirectRecording() {
    try {
      ...
      const activeView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian20.MarkdownView);
      if (!activeView || !activeView.file) {
        new import_obsidian20.Notice("No active file to insert recording");
        return;
      }
      await this.plugin.recordingProcessor.processRecording(
        blob,
      ...
      new import_obsidian20.Notice("Failed to stop recording");
    } finally {
      this.cleanup();
    }
  }
  ...
  async handleDroppedAudio(dataTransfer, source) {
    ...
    if (!valid.includes(ext) && !file.type.startsWith("audio/")) {
      new import_obsidian20.Notice("Dropped file is not a supported audio format.");
      return;
    }
    const activeView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian20.MarkdownView);
    if (!activeView?.file) {
      new import_obsidian20.Notice("Open a note first to insert dropped audio transcription.");
      return;
    }
    ...
    try {
      ...
      new import_obsidian20.Notice(`Transcribed dropped audio: ${file.name}`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian20.Notice(`Failed to transcribe dropped audio: ${message}`);
    } finally {
      this.isProcessing = false;
      this.updateProcessingState(false);
    }
  ...
  async toggleInlineRecorderPanel() {
    ...
    const activeView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian20.MarkdownView);
    if (!activeView || !activeView.file) {
      new import_obsidian20.Notice("No active note found to insert transcription.");
      return;
    }
    if (!this.activeLeafContainer) {
      new import_obsidian20.Notice("Recorder panel unavailable in this view.");
      return;
    }
  ...
  async ensureMobileLiveDefaults() {
    ...
    try {
      await this.plugin.saveSettings({ refreshUi: false, triggerFloatingRefresh: false });
      new import_obsidian20.Notice("Enabled mobile speaker-turn timestamps by default.");
    } catch (e) {
    }
  }
};
...
var _TimerModal = class extends import_obsidian21.Modal {
  ...
  handleLifecycleInterruption(_reason) {
    ...
    this.pauseRecording("Recording paused due to app interruption. Resume when back.");
  }
  handleLifecycleResume() {
    if (this.currentState !== "paused" || !this.interruptedByLifecycle)
      return;
    void this.startRecording();
  }
...
var RecoveryJobsModal = class extends import_obsidian22.Modal {
...
var OpenAIAdapter = class extends AIAdapter {
  ...
  async validateApiKeyImpl() {
    ...
    try {
      ...
      if (err?.status === 401) {
        return { valid: false, error: message, errorType: "auth" };
      }
      if (message.toLowerCase().includes("timeout") || message.toLowerCase().includes("timed out")) {
        return { valid: false, error: message, errorType: "timeout" };
      }
      if (message.toLowerCase().includes("network") || message.toLowerCase().includes("fetch") || message.toLowerCase().includes("econnrefused")) {
        return { valid: false, error: message, errorType: "network" };
      }
      return { valid: false, error: message, errorType: "unknown" };
    }
  }
...
var _DeepgramAdapter = class extends AIAdapter {
  ...
  async validateApiKeyImpl() {
    ...
    try {
      ...
      return { valid: false, error: "Unexpected response format", errorType: "unknown" };
    } catch (error) {
      const err = error;
      const message = err?.message || (error instanceof Error ? error.message : String(error));
      if (err?.status === 401) {
  ...
  parseTextGenerationResponse(_response) {
  ...
  /**
   * Parses a Deepgram transcription response with optional handling for empty transcriptions.
   * When speaker diarization is enabled, formats utterances with speaker labels and timestamps.
   * Falls back to channel-level transcript if no utterances are present.
   */
   ...
   * Sends raw audio bytes directly (not multipart form data).
   * Applies dynamic timeout based on file size.
   */
  async transcribeAudio(audioArrayBuffer, model, options) {
    try {
      ...
      const transcript = this.parseTranscriptionResponseWithOptions(
        response,
        options?.allowEmptyTranscription === true
      );
      if (!options?.allowEmptyTranscription && (!transcript || !transcript.trim())) {
  ...
  /**
   * Diagnostic endpoint: sends audio to Deepgram and returns detailed metadata
   * about the response structure, useful for debugging diarization and language issues.
   */
  async diagnoseAudio(audioArrayBuffer, model, options) {
    ...
    const utterances = Array.isArray(response?.results?.utterances) ? response.results.utterances : [];
    const channel = response?.results?.channels?.[0];
    const alternatives = Array.isArray(channel?.alternatives) ? channel.alternatives : [];
    const transcript = (typeof alternatives?.[0]?.transcript === "string" ? alternatives[0].transcript : "") || "";
    ...
    return {
      ...
      channelsCount: Array.isArray(response?.results?.channels) ? response.results.channels.length : 0,
  ...
  /**
   * Calculates a dynamic timeout based on file size in bytes.
   * Scales linearly at TIMEOUT_PER_MB_MS per megabyte, clamped between MIN and MAX.
   */
  ...
  /**
   * Applies language hint settings to the query parameters.
   * Supports:
   * - Multi-language mode for Nova-2/3 models
   * - Language detection with hint list
  ...
  /**
   * Wraps a Deepgram API request with a timeout that rejects after timeoutMs.
   */
  ...
  /**
   * Formats diarized utterances into a readable transcript with speaker labels
   * and timestamp tokens. Consecutive utterances from the same speaker are merged.
   */
  formatDiarizedTranscript(utterances) {
    let currentSpeaker = null;
    const lines = [];
    for (const utt of utterances) {
      const text = typeof utt?.transcript === "string" ? utt.transcript.trim() : "";
      if (!text)
        continue;
      const speakerId = typeof utt?.speaker === "number" ? utt.speaker + 1 : 0;
      const speakerLabel = speakerId > 0 ? `Speaker ${speakerId}` : "Speaker";
      const timestamp = this.formatTimestampToken(utt?.start);
  ...
  /**
   * Normalizes speaker formatting in the transcript by ensuring each speaker label
   * starts on a new line and removing excessive blank lines.
   */
  normalizeSpeakerFormatting(transcript) {
  ...
  /**
   * Formats a start-time in seconds into an [HH:MM:SS] timestamp token.
   */
  ...
  /**
   * Override makeAPIRequest to use Deepgram's "Token" authorization header
  ...
  /**
   * Extracts a compact, human-readable error message from Deepgram errors.
   * Includes HTTP status code when available and truncates long messages.
   */
...
var CURRENT_SETTINGS_VERSION2 = 6;
var OLD_DEFAULT_POST_PROCESSING_PROMPT = "Process the following transcript to extract key insights and information.";
var OLD_DEFAULT_POST_PROCESSING_MAX_TOKENS = 500;
function migrateAndNormalizeSettings(data) {
  const raw = isRecord(data) ? data : {};
  const sourceVersion = getSourceVersion(raw);
  const warnings = [];
  const migrated = sourceVersion < CURRENT_SETTINGS_VERSION2;
  const merged = {
    ...DEFAULT_SETTINGS,
    ...raw
  };
  const settings = {
    ...DEFAULT_SETTINGS,
    settingsVersion: CURRENT_SETTINGS_VERSION2,
    ...
    // Meeting Templates (M3)
    defaultMeetingTemplate: asString(merged.defaultMeetingTemplate, DEFAULT_SETTINGS.defaultMeetingTemplate),
    enableAutoTemplateDetection: asBoolean(
      merged.enableAutoTemplateDetection,
      DEFAULT_SETTINGS.enableAutoTemplateDetection
    ),
    enableEnhanceNotes: asBoolean(merged.enableEnhanceNotes, DEFAULT_SETTINGS.enableEnhanceNotes),
    addMeetingFrontmatter: asBoolean(merged.addMeetingFrontmatter, DEFAULT_SETTINGS.addMeetingFrontmatter),
    customMeetingTemplates: asString(merged.customMeetingTemplates, DEFAULT_SETTINGS.customMeetingTemplates),
    // Inbox Auto-Watch
    enableInboxAutoWatch: asBoolean(merged.enableInboxAutoWatch, DEFAULT_SETTINGS.enableInboxAutoWatch),
    autoTranscribeInbox: asBoolean(merged.autoTranscribeInbox, DEFAULT_SETTINGS.autoTranscribeInbox),
    archiveAfterTranscribe: asBoolean(merged.archiveAfterTranscribe, DEFAULT_SETTINGS.archiveAfterTranscribe)
  };
  ...
  if (sourceVersion <= 5) {
    const rawMaxTokens = raw.postProcessingMaxTokens;
    if (typeof rawMaxTokens === "number" && rawMaxTokens === OLD_DEFAULT_POST_PROCESSING_MAX_TOKENS) {
      settings.postProcessingMaxTokens = DEFAULT_SETTINGS.postProcessingMaxTokens;
      warnings.push("postProcessingMaxTokens upgraded from 500 to 2000 (new default for meeting templates)");
    }
    const rawPrompt = raw.postProcessingPrompt;
    if (typeof rawPrompt === "string" && rawPrompt.trim() === OLD_DEFAULT_POST_PROCESSING_PROMPT) {
      settings.postProcessingPrompt = DEFAULT_SETTINGS.postProcessingPrompt;
      warnings.push("postProcessingPrompt upgraded to new structured default");
    }
  }
  return {
    settings,
...
// src/main.ts
var _NeuroVoxPlugin = class extends import_obsidian24.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.events = new import_obsidian24.Events();
    ...
    this.recordingIndicatorTimerId = null;
    this.recordingIndicatorStartedAt = null;
  ...
  async onload() {
    ...
    try {
      ...
      new import_obsidian24.Notice("Failed to initialize NeuroVox plugin");
    }
  }
  /**
    ...
    try {
      ...
      if (migration.migrated) {
        ...
        new import_obsidian24.Notice(
          `NeuroVox settings migrated to v${CURRENT_SETTINGS_VERSION2} from v${migration.sourceVersion}.`
        );
      }
      if (migration.warnings.length > 0) {
        new import_obsidian24.Notice("NeuroVox normalized invalid settings values.");
      }
    } catch (error) {
      this.settings = { ...DEFAULT_SETTINGS };
      new import_obsidian24.Notice("Failed to load NeuroVox settings. Using defaults.");
    }
  }
  /**
  ...
  async saveSettings(options) {
    try {
      this.settings.settingsVersion = CURRENT_SETTINGS_VERSION2;
      await this.saveData(this.settings);
      const refreshUi = options?.refreshUi ?? true;
      const triggerFloatingRefresh = options?.triggerFloatingRefresh ?? true;
      if (refreshUi) {
        this.initializeUI();
      }
      ...
      new import_obsidian24.Notice("Failed to save NeuroVox settings");
    }
  }
  async validateApiKeys(showInvalidNotices = true) {
    try {
      ...
      if (showInvalidNotices && openaiAdapter && !openaiAdapter.isReady() && this.settings.openaiApiKey) {
        new import_obsidian24.Notice("\u274C OpenAI API key validation failed");
      }
      if (showInvalidNotices && groqAdapter && !groqAdapter.isReady() && this.settings.groqApiKey) {
        new import_obsidian24.Notice("\u274C Groq API key validation failed");
      }
      if (showInvalidNotices && deepgramAdapter && !deepgramAdapter.isReady() && this.settings.deepgramApiKey) {
        new import_obsidian24.Notice("\u274C Deepgram API key validation failed");
      }
    } catch (error) {
    }
  }
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (!activeFile || !this.isValidAudioFile(activeFile)) {
          new import_obsidian24.Notice("\u274C Active file is not a valid audio file");
          return;
        }
        new import_obsidian24.Notice(`\u{1F3B5} Transcribing: ${activeFile.path}`);
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          if (jobs.length === 0) {
            new import_obsidian24.Notice("NeuroVox queue is empty.");
            return;
          }
          ...
          new import_obsidian24.Notice(`NeuroVox queue: ${summary}`, 12e3);
          const top = jobs.slice(0, 3).map(
            (j) => `${j.id} ${j.status} attempts=${j.attemptCount}`
          );
          if (top.length > 0) {
            new import_obsidian24.Notice(`Recent jobs:
${top.join("\n")}`, 12e3);
          }
        } catch (error) {
          new import_obsidian24.Notice("Failed to inspect NeuroVox queue.");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          if (pending.length === 0) {
            new import_obsidian24.Notice("NeuroVox has no incomplete jobs to cancel.");
            return;
          }
          await Promise.all(pending.map((job) => this.recordingProcessor.cancelJob(job.jobId)));
          await this.reconcileProcessingStatusFromJobs();
          new import_obsidian24.Notice(`NeuroVox canceled ${pending.length} incomplete job(s).`);
        } catch (error) {
          new import_obsidian24.Notice("Failed to cancel incomplete NeuroVox jobs.");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        try {
          ...
          new import_obsidian24.Notice("NeuroVox recovery review failed.");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        try {
          if (this.settings.transcriptionProvider !== "deepgram" /* Deepgram */) {
            new import_obsidian24.Notice("Set transcription provider to Deepgram before running diagnosis.");
            return;
          }
          const deepgram = this.aiAdapters.get("deepgram" /* Deepgram */);
          if (!(deepgram instanceof DeepgramAdapter)) {
            new import_obsidian24.Notice("Deepgram adapter is not available.");
            return;
          }
          if (!deepgram.getApiKey()) {
            new import_obsidian24.Notice("Deepgram API key is not configured.");
            return;
          }
          const activeView = this.app.workspace.getActiveViewOfType(import_obsidian24.MarkdownView);
          const activeFile = activeView?.file;
          if (!activeFile) {
            new import_obsidian24.Notice("Open a transcript note with source metadata first.");
            return;
          }
          const content = await this.app.vault.read(activeFile);
          const sourcePath = this.resolveSourcePathFromNote(content);
          if (!sourcePath) {
            new import_obsidian24.Notice("No source path found in this note.");
            return;
          }
          const adapter = this.app.vault.adapter;
          const normalizedSource = (0, import_obsidian24.normalizePath)(sourcePath);
          if (!await adapter.exists(normalizedSource)) {
            new import_obsidian24.Notice(`Source audio file not found: ${normalizedSource}`);
            return;
          }
          ...
          new import_obsidian24.Notice(`Deepgram diagnosis saved: ${reportPath}`, 1e4);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          new import_obsidian24.Notice(`Deepgram diagnosis failed: ${message}`, 1e4);
        }
      }
    });
    this.addCommand({
    ...
    this.addCommand({
      id: "re-enhance-notes",
      name: "Re-enhance meeting notes",
      checkCallback: (checking) => {
        const activeView = this.app.workspace.getActiveViewOfType(import_obsidian24.MarkdownView);
        if (!activeView?.file)
          return false;
        const content = activeView.editor.getValue();
        if (!content.includes("<!-- neurovox:ai:start:"))
          return false;
        if (checking)
          return true;
        void this.reEnhanceCurrentNote(activeView);
        return true;
      }
    });
  }
  ...
  isFileWithinFolder(filePath, folderPath) {
    ...
    const normalizedFolder = (0, import_obsidian24.normalizePath)(folderPath).replace(/\/+$/, "");
    if (!normalizedFolder)
      return false;
  ...
  async transcribeLatestIphoneInboxRecording() {
    try {
      const inboxFolder = (0, import_obsidian24.normalizePath)(
        (this.settings.iphoneInboxFolderPath || "Recordings/Inbox").trim()
      );
      const adapter = this.app.vault.adapter;
      if (!await adapter.exists(inboxFolder)) {
        new import_obsidian24.Notice(`iPhone inbox folder not found: ${inboxFolder}`);
        return;
      }
      const candidates = this.app.vault.getFiles().filter((file) => this.isSupportedInboxAudioFile(file) && this.isFileWithinFolder(file.path, inboxFolder)).sort((a, b) => b.stat.mtime - a.stat.mtime);
      if (candidates.length === 0) {
        new import_obsidian24.Notice(`No audio files found in iPhone inbox: ${inboxFolder}`);
        return;
      }
      const latestFile = candidates[0];
      new import_obsidian24.Notice(`\u{1F3B5} Transcribing latest inbox file: ${latestFile.name}`);
      await this.processExistingAudioFile(latestFile);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian24.Notice(`Failed to transcribe latest iPhone inbox recording: ${message}`);
    }
  }
  ...
  async processExistingAudioFile(file) {
    try {
      ...
      let count = 1;
      const normalizedPath = (0, import_obsidian24.normalizePath)(transcriptsFolder);
      ...
      new import_obsidian24.Notice("\u{1F399}\uFE0F Processing audio file...");
      ...
      new import_obsidian24.Notice("\u2728 Transcription completed successfully!");
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
  ...
  async processVideoFile(file) {
    try {
      ...
      new import_obsidian24.Notice("\u274C Failed to process video file");
      throw error;
    }
  }
  registerEvents() {
    ...
    this.registerEvent(
      this.app.vault.on("create", this.handleInboxFileCreate.bind(this))
    );
  }
  registerTranscriptTimestampSeekProcessor() {
    this.registerMarkdownPostProcessor((el, ctx) => {
      ...
      this.bindLiveCalloutFoldStateCapture(el, ctx?.sourcePath);
    });
  }
  ...
  isLiveTranscriptionCallout(callout) {
    const titleText = callout.querySelector(".callout-title-inner")?.textContent?.trim().toLowerCase() || "";
    const bodyText = callout.textContent?.toLowerCase() || "";
  ...
  seekNearestAudioFromToken(tokenEl, targetSeconds) {
    ...
    if (!audio) {
      new import_obsidian24.Notice("No audio player found in this note.");
      return;
    }
    try {
      ...
      new import_obsidian24.Notice("Could not seek audio for this timestamp.");
    }
  }
  ...
  handleActiveLeafChange(leaf) {
    ...
    this.buttonMap.forEach((button, _path) => {
      button.remove();
    });
    this.buttonMap.clear();
    if (this.settings.showFloatingButton && leaf?.view instanceof import_obsidian24.MarkdownView && leaf.view.file) {
  ...
  /**
   * Handles new files created in the vault. If inbox auto-watch is enabled
   * and the file is an audio file in the inbox folder, triggers transcription.
   */
  handleInboxFileCreate(file) {
    if (!(file instanceof import_obsidian24.TFile))
      return;
    if (!this.settings.enableInboxAutoWatch)
      return;
    if (!this.isSupportedInboxAudioFile(file))
      return;
    const inboxFolder = (0, import_obsidian24.normalizePath)(
      (this.settings.iphoneInboxFolderPath || "Recordings/Inbox").trim()
    );
    if (!this.isFileWithinFolder(file.path, inboxFolder))
      return;
    window.setTimeout(() => {
      void this.handleDetectedInboxAudio(file);
    }, 2e3);
  }
  async handleDetectedInboxAudio(file) {
    const stillExists = this.app.vault.getAbstractFileByPath(file.path);
    if (!stillExists)
      return;
    if (this.settings.autoTranscribeInbox) {
      new import_obsidian24.Notice(`NeuroVox: Auto-transcribing ${file.name}`);
      await this.transcribeInboxFile(file);
    } else {
      const notice = new import_obsidian24.Notice("", 0);
      const frag = document.createDocumentFragment();
      const msg = frag.createDiv({ cls: "neurovox-inbox-notice" });
      msg.createDiv({
        cls: "neurovox-inbox-notice-title",
        text: `New recording: ${file.name}`
      });
      const btnRow = msg.createDiv({ cls: "neurovox-inbox-notice-actions" });
      const transcribeBtn = btnRow.createEl("button", {
        cls: "mod-cta",
        text: "Transcribe"
      });
      ...
      transcribeBtn.addEventListener("click", () => {
        notice.hide();
        void this.transcribeInboxFile(file);
      });
  ...
  async transcribeInboxFile(file) {
    try {
      await this.processExistingAudioFile(file);
      new import_obsidian24.Notice(`NeuroVox: Transcription complete for ${file.name}`);
      if (this.settings.archiveAfterTranscribe) {
        const inboxFolder = (0, import_obsidian24.normalizePath)(
          (this.settings.iphoneInboxFolderPath || "Recordings/Inbox").trim()
        );
        const archiveFolder = `${inboxFolder}/Archive`;
      ...
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian24.Notice(`NeuroVox: Failed to transcribe ${file.name}: ${message}`);
    }
  }
  initializeUI() {
    this.cleanupUI();
  ...
  handleRecordingStart() {
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian24.MarkdownView);
    if (!activeView) {
      new import_obsidian24.Notice("\u274C No active note found to insert transcription.");
      return;
    }
    const activeFile = activeView.file;
    if (!activeFile) {
      new import_obsidian24.Notice("\u274C No active file found.");
      return;
    }
    ...
    if (this.settings.useRecordingModal) {
      ...
      this.modalInstance.onStop = async (result) => {
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : "Unknown error";
          new import_obsidian24.Notice(`\u274C Failed to process recording: ${errorMessage}`);
        }
      };
  ...
  async runDeferredMaintenanceAndRecovery() {
    ...
    try {
      ...
      new import_obsidian24.Notice("NeuroVox startup maintenance skipped due to storage error.");
    }
    try {
      const pending = await this.recordingProcessor.getIncompleteJobs();
      if (pending.length > 0) {
      ...
      new import_obsidian24.Notice("NeuroVox recovery scan failed at startup.");
    } finally {
      const elapsed = Math.round(performance.now() - startedAt);
  ...
  async openRecoveryJobsModal() {
    try {
      const pending = await this.recordingProcessor.getIncompleteJobs();
      if (pending.length === 0) {
        new import_obsidian24.Notice("NeuroVox has no incomplete jobs.");
        return;
      }
  ...
  async executeRecoveryAction(action, ordered) {
    ...
    if (action.type === "cancel_all") {
      ...
      await this.reconcileProcessingStatusFromJobs();
      new import_obsidian24.Notice("NeuroVox canceled all incomplete transcription jobs.");
      return;
    }
    ...
    if (action.type === "cancel") {
      ...
      await this.reconcileProcessingStatusFromJobs();
      new import_obsidian24.Notice("NeuroVox canceled the selected incomplete transcription job.");
    }
  }
  async validateAdapterWithTimeout(adapter) {
    const result = await Promise.race([
      adapter.validateApiKey().catch(() => ({ valid: false })),
      new Promise((resolve) => {
        window.setTimeout(() => resolve({ valid: false }), _NeuroVoxPlugin.STARTUP_VALIDATION_TIMEOUT_MS);
      })
    ]);
    return result.valid;
  }
  ...
  async persistSettingsBackup(source) {
    ...
    try {
      ...
      new import_obsidian24.Notice("NeuroVox could not write settings backup before migration.");
    }
  }
  ...
  /**
   * Shows a persistent recording indicator (red dot + elapsed time) in the status bar.
   * Visible across all Obsidian views regardless of active note.
   */
  showRecordingIndicator() {
    if (!this.recordingIndicatorEl) {
      this.recordingIndicatorEl = this.addStatusBarItem();
      this.recordingIndicatorEl.addClass("neurovox-recording-indicator");
    }
    this.recordingIndicatorStartedAt = Date.now();
    this.renderRecordingIndicator();
    this.recordingIndicatorEl.show();
    if (this.recordingIndicatorTimerId === null) {
      this.recordingIndicatorTimerId = window.setInterval(() => {
        this.renderRecordingIndicator();
      }, 1e3);
    }
  }
  /**
   * Hides the recording indicator from the status bar.
   */
  hideRecordingIndicator() {
    if (this.recordingIndicatorTimerId !== null) {
      window.clearInterval(this.recordingIndicatorTimerId);
      this.recordingIndicatorTimerId = null;
    }
    this.recordingIndicatorStartedAt = null;
    this.recordingIndicatorEl?.hide();
  }
  renderRecordingIndicator() {
    if (!this.recordingIndicatorEl || !this.recordingIndicatorStartedAt)
      return;
    const elapsedMs = Date.now() - this.recordingIndicatorStartedAt;
    const totalSec = Math.floor(elapsedMs / 1e3);
    const minutes = Math.floor(totalSec / 60).toString().padStart(2, "0");
    const seconds = (totalSec % 60).toString().padStart(2, "0");
    this.recordingIndicatorEl.empty();
    const dot = this.recordingIndicatorEl.createSpan({ cls: "neurovox-recording-dot" });
    dot.setAttribute("aria-hidden", "true");
    this.recordingIndicatorEl.createSpan({ text: ` ${minutes}:${seconds}` });
    this.recordingIndicatorEl.setAttribute("aria-label", `Recording in progress: ${minutes}:${seconds}`);
  }
  extractSourcePathFromFrontmatter(content) {
    ...
    const sourceLine = frontmatter.split("\n").find(
      (line) => line.trim().toLowerCase().startsWith("source:")
    );
    if (!sourceLine)
      return null;
  ...
  async applySpeakerNamesFromMappingInActiveNote() {
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian24.MarkdownView);
    const activeFile = activeView?.file;
    if (!(activeFile instanceof import_obsidian24.TFile) || !activeView || activeFile.extension.toLowerCase() !== "md") {
      new import_obsidian24.Notice("Open a transcript markdown note first.");
      return;
    }
    try {
      const content = await this.app.vault.read(activeFile);
      const region = findEntryRegionAtPosition(content, activeView.editor.getCursor());
      if (!region || !region.meta?.id) {
        new import_obsidian24.Notice("No scoped transcription entry mapping found at cursor.");
        return;
      }
      const result = applySpeakerMappingToEntry(content, region.meta.id, region.start, region.end);
      if (result.mappedSpeakers === 0) {
        new import_obsidian24.Notice("No speaker names found in the Speaker Mapping section.");
        return;
      }
      if (result.replacedCount === 0) {
        new import_obsidian24.Notice("No matching speaker labels found in this entry body. Ensure lines use speaker labels (e.g., Speaker 1:).");
        return;
      }
      await this.app.vault.modify(activeFile, result.updatedContent);
      new import_obsidian24.Notice(`Applied ${result.replacedCount} speaker label replacement(s).`);
    } catch (e) {
      new import_obsidian24.Notice("Failed to apply speaker names from mapping.");
    }
  }
  handleNoteModifyForSpeakerAutoApply(file) {
    if (!(file instanceof import_obsidian24.TFile) || file.extension.toLowerCase() !== "md")
      return;
  ...
  async autoApplySpeakerMappingForPath(path) {
    ...
    if (!(abstract instanceof import_obsidian24.TFile) || abstract.extension.toLowerCase() !== "md")
      return;
    ...
    try {
      ...
      if (!hasSpeakerMappingSection(content))
        return;
      const entries = findEntryRegions(content).filter((entry) => !!entry.meta?.id);
      if (entries.length === 0)
        return;
      let updated = content;
      let totalReplaced = 0;
  ...
  async generateSpeakerMappingForActiveNote() {
    ...
    try {
      ...
      const region = findEntryRegionAtPosition(content, activeView.editor.getCursor());
      if (!region || !region.meta?.id) {
        new import_obsidian24.Notice("No transcription entry found at cursor.");
        return;
      }
      const entryContent = content.slice(region.start, region.end);
      if (hasEntrySpeakerMappingSection(entryContent, region.meta.id)) {
        new import_obsidian24.Notice("Speaker Mapping section already exists in this transcription.");
        return;
      }
      const labels = extractSpeakerLabels(entryContent);
      if (labels.length === 0) {
        new import_obsidian24.Notice("No diarized speaker labels found in this transcription.");
        return;
      }
...
`;
      ...
      if (!calloutHeaderMatch || calloutHeaderMatch.index === void 0) {
        new import_obsidian24.Notice("Could not find transcription callout in this entry.");
        return;
      }
      ...
      new import_obsidian24.Notice(`Speaker Mapping created for ${labels.length} speaker(s).`);
    } catch (e) {
      new import_obsidian24.Notice("Failed to generate Speaker Mapping for this note.");
    }
  }
  async renameCurrentTranscriptionEntry() {
    ...
    if (!newTitle) {
      new import_obsidian24.Notice("Title cannot be empty.");
      return;
    }
    try {
      const content = await this.app.vault.read(activeFile);
      ...
      if (updatedEntry === entry) {
        new import_obsidian24.Notice("Could not update this transcription entry title.");
        return;
      }
      const updated = `${content.slice(0, region.start)}${updatedEntry}${content.slice(region.end)}`;
      await this.app.vault.modify(activeFile, updated);
      new import_obsidian24.Notice("Transcription entry title updated.");
    } catch (e) {
      new import_obsidian24.Notice("Failed to rename transcription entry.");
    }
  }
  /**
   * Re-enhances AI sections in the current note.
   * Finds AI section markers, checks for user edits via hash comparison,
   * and re-runs enhancement using the transcript from the note.
   */
  async reEnhanceCurrentNote(view) {
    const file = view.file;
    if (!file)
      return;
    try {
      const content = view.editor.getValue();
      const sectionRegex = /<!-- neurovox:ai:start:section=(\w+);hash=([a-f0-9]+) -->/g;
      let hasEditedSections = false;
      let match;
      while ((match = sectionRegex.exec(content)) !== null) {
        const sectionId = match[1];
        const storedHash = match[2];
        const startMarker = match[0];
        const endMarker = `<!-- neurovox:ai:end:section=${sectionId} -->`;
        const startIdx = content.indexOf(startMarker) + startMarker.length;
        const endIdx = content.indexOf(endMarker);
        if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx)
          continue;
        const currentContent = content.slice(startIdx, endIdx).split("\n").map((line) => line.replace(/^>\s?/, "")).join("\n").trim();
        const currentHash = this.computeContentHash(currentContent);
        if (currentHash !== storedHash) {
          hasEditedSections = true;
          break;
        }
      }
      if (hasEditedSections) {
        const proceed = window.confirm(
          "Some AI-generated sections have been edited. Re-enhancing will overwrite your changes. Continue?"
        );
        if (!proceed)
          return;
      }
      const transcriptMatch = content.match(
        />\s*\[!info\]-?\s*Full Transcript\n([\s\S]*?)(?=\n(?:>?\s*\[!|\n##\s|$))/
      );
      if (!transcriptMatch) {
        new import_obsidian24.Notice("No Full Transcript section found in this note.");
        return;
      }
      const transcript = transcriptMatch[1].split("\n").map((line) => line.replace(/^>\s?/, "")).join("\n").trim();
      if (!transcript) {
        new import_obsidian24.Notice("Full Transcript section is empty.");
        return;
      }
      new import_obsidian24.Notice("Re-enhancing meeting notes...");
      this.showProcessingStatus("Re-enhancing notes");
      const { getBuiltInTemplate: getBuiltInTemplate2 } = await Promise.resolve().then(() => (init_MeetingTemplates(), MeetingTemplates_exports));
      const template = getBuiltInTemplate2(this.settings.defaultMeetingTemplate);
      const adapter = this.aiAdapters.get(this.settings.postProcessingProvider);
      if (!adapter) {
        new import_obsidian24.Notice("AI adapter not available for re-enhancement.");
        return;
      }
      const userPrompt = template.userPromptTemplate.replace("{transcript}", transcript).replace("{user_notes}", "");
      const postProcessing = await adapter.generateResponse(
        userPrompt,
        this.settings.postProcessingModel,
        {
          maxTokens: this.settings.postProcessingMaxTokens,
          temperature: this.settings.postProcessingTemperature,
          systemPrompt: template.systemPrompt
        }
      );
      const { StructuredOutputRenderer: StructuredOutputRenderer2 } = await Promise.resolve().then(() => (init_StructuredOutputRenderer(), StructuredOutputRenderer_exports));
      const sectionDefs = template.sections.map((s) => ({
        id: s.id,
        heading: s.heading,
        calloutType: s.calloutType
      }));
      const newSections = StructuredOutputRenderer2.parseSectionsFromPostProcessing(
        postProcessing,
        sectionDefs
      );
      let updatedContent = content;
      for (const section of newSections) {
        const newHash = this.computeContentHash(section.content);
        const startPattern = new RegExp(
          `<!-- neurovox:ai:start:section=${section.id};hash=[a-f0-9]+ -->`
        );
        const endPattern = `<!-- neurovox:ai:end:section=${section.id} -->`;
        const startMatch = startPattern.exec(updatedContent);
        if (!startMatch)
          continue;
        const startIdx = startMatch.index;
        const endIdx = updatedContent.indexOf(endPattern, startIdx);
        if (endIdx === -1)
          continue;
        const newStartMarker = `<!-- neurovox:ai:start:section=${section.id};hash=${newHash} -->`;
        const newContentLines = section.content.split("\n").map((line) => line.trim() ? `> ${line}` : ">").join("\n");
        const replacement = `${newStartMarker}
${newContentLines}
> `;
      ...
      this.setProcessingStatus("Idle");
      new import_obsidian24.Notice("Meeting notes re-enhanced.");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian24.Notice(`Re-enhancement failed: ${message}`);
      this.setProcessingStatus("Idle");
    }
  }
  computeContentHash(content) {
  ...
  async writeDeepgramDiagnosticReport(diagnosis, sourcePath) {
    const adapter = this.app.vault.adapter;
    const baseDir = (0, import_obsidian24.normalizePath)("nuerovox/diagnostics");
    if (!await adapter.exists(baseDir)) {
      await adapter.mkdir(baseDir);
    }
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    const filePath = (0, import_obsidian24.normalizePath)(`${baseDir}/deepgram-diagnosis-${stamp}.md`);
    const body = [
      "---",
